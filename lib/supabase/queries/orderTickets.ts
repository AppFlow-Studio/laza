...
export async function createTicket(input: CreateTicketInput) {
	...
	const { data: ticket, error: ticketError } = await supabase
		...
		.insert({
			...
			submitted_at:
				input.initialStatus === "submitted" ? new Date().toISOString() : null,
		})
		.select("id, status")
		.single();
...
export async function getTicketById(id: string) {
    ...
    `,
		...
		.eq("id", id)
		.order("created_at", { referencedTable: "order_ticket_logs", ascending: true })
		.single();
...
export async function getTicketLogs(ticketId: string) {
	...
	const { data, error } = await supabase
		...
		.select("id, ticket_id, previous_status, new_status, changed_by, notes, created_at")
		.eq("ticket_id", ticketId)
...
export async function updateTicketStatus(
	...
	const terminalStatuses: TicketStatus[] = ["confirmed", "cancelled", "rejected"];
...
export async function cancelTicket(ticketId: string, userId: string) {
	...
	await insertTicketLog(supabase, ticketId, "submitted", "cancelled", userId, "Cancelled by store admin");
...
export async function confirmTicket(
	...
	for (const received of receivedItems) {
		...
		if (actualBoxes !== expectedBoxes) {
			discrepancies.push({ itemId: received.itemId, itemName, expectedBoxes, actualBoxes });
		}
	}
	...
	const logNote = hasDiscrepancy
		? `Confirmed with discrepancies — ${discrepancies
			.map((d) => `${d.itemName}: expected ${d.expectedBoxes} boxes, received ${d.actualBoxes} boxes`)
			.join(" | ")}`
