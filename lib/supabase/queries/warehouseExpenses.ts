...
type WarehouseExpense = Database['public']['Tables']['warehouse_expenses']['Row'];
type WarehouseExpenseInsert = Database['public']['Tables']['warehouse_expenses']['Insert'];
type ExpenseRate = Database['public']['Tables']['warehouse_expense_rates']['Row'];
...
export async function getExpenseSummary(
  ...
  for (const row of data ?? []) {
    ...
    if (existing) {
      ...
      summaryMap.set(type, { expense_type: type, total_amount: Number(row.amount), entry_count: 1 });
    }
  }
 ...
 */
export async function getExpenseRates(organizationId: string): Promise<ExpenseRate[]> {
  const supabase = createServerSupabaseClient();
