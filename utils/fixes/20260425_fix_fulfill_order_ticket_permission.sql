-- Fix: fulfill_order_ticket (4-param) has no super_admin permission check.
--
-- Root cause: the function is GRANTED to `authenticated` (every logged-in user)
-- and the function body never verifies the caller has the super_admin role.
-- A store admin who knows the RPC signature can call it directly via the
-- Supabase client and fulfill any order in the organization.
--
-- Fix: add `IF NOT is_super_admin()` guard at the very start of the function,
-- identical to the pattern already used in receive_shipment_to_pallets.
-- The GRANT is left as-is (authenticated) so the call can reach the function
-- and get a clean PERMISSION_DENIED rather than a 404/unknown-function error.
--
-- DROP is required before CREATE OR REPLACE because the original definition had
-- DEFAULT values on p_allow_partial and p_delivery_type; Postgres forbids
-- removing defaults via CREATE OR REPLACE.
...
BEGIN
  ...
  -- ── 0. Permission check ──────────────────────────────────────────────────────
  -- Only super_admin may fulfill orders. The check uses is_super_admin() which
  -- reads the caller's JWT (auth.jwt() ->> 'sub') regardless of SECURITY DEFINER.
  ...
  -- FIX from 20260418: was LEFT JOIN; warehouse filter was in ON clause so stock
  -- from other locations was counted. Fixed to INNER JOIN with WHERE clause.
...
-- Re-apply grant after DROP (DROP removes all existing grants).
