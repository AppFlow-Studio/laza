-- Fix: receive_shipment_to_pallets was JOIN-scanning for any warehouse in the org
-- instead of using po.warehouse_location_id stored on the purchase order.
-- This caused all pallets to land on whichever warehouse the JOIN returned first.
...
BEGIN
  ...
  -- ── 1. Load PO context ───────────────────────────────────
  -- Use po.warehouse_location_id directly — do NOT join locations,
  -- which previously returned an arbitrary warehouse for the org.
  SELECT po.organization_id, po.warehouse_location_id
