-- Fix: receive_shipment_to_pallets raises PO_NO_WAREHOUSE when
-- po.warehouse_location_id is NULL (POs created before the warehouse_location_id
-- column was populated). The previous version resolved the warehouse via a JOIN,
-- which worked even with NULL. Restore that fallback while keeping the direct
-- warehouse_location_id path when it is set.
...
BEGIN
  ...
  -- Use po.warehouse_location_id when set; fall back to the first warehouse
  -- in the org for POs that were created before this column was populated.
  ...
  --
  -- When Phase A (receive_purchase_order) ran first, item_locations rows
  -- already exist and the NOT EXISTS guard makes this a complete no-op.
  --
  -- When the wizard opens at step 2 (PO already 'received', no pallets),
  -- Phase A was skipped. Insert item_locations rows here so the warehouse
  -- inventory tab shows the received stock.
