-- Fix: fulfill_order_ticket permission check uses is_super_admin() which reads
-- the Clerk JWT from request.jwt.claims. The mutation in useOrderTickets.ts
-- creates a plain Supabase client (no accessToken callback), so the JWT is
-- never attached and requesting_user_id() returns NULL → is_super_admin()
-- always returns false → every super_admin call fails with PERMISSION_DENIED.
--
-- receive_shipment_to_pallets avoids this correctly by checking p_user_id
-- directly against the users table. Apply the same pattern here.
--
-- DROP required because the previous fix added DEFAULT-less params
-- (replacing the version that had DEFAULT values), so CREATE OR REPLACE
-- would fail trying to change the signature.
...
BEGIN
  ...
  -- ── 0. Permission check ──────────────────────────────────────────────────────
  -- Use p_admin_user_id directly (same pattern as receive_shipment_to_pallets).
  -- is_super_admin() reads from request.jwt.claims which is empty when the
  -- client doesn't attach a Clerk token, so it cannot be trusted here.
  IF NOT EXISTS (
    SELECT 1 FROM users
