-- Fix: fulfill_order_ticket (4-param) was not updating item_locations.
--
-- Root cause: the function relied on a "deferred sync trigger" that does not
-- exist. Neither overload directly updates item_locations.
--
-- This migration replaces the 4-param overload with a version that:
--   1. Adds UPDATE item_locations after each item's FIFO deduction.
--   2. Fixes the LEFT JOIN pre-check bug (warehouse filter was in ON clause,
--      so stock from other locations was counted).
...
BEGIN
  ...
  -- ================================================================
  -- Pre-check: if full fulfillment required, verify all items have
  -- sufficient stock in THIS warehouse before touching anything.
  -- FIX: was LEFT JOIN — warehouse filter was ignored, counting stock
  --      from other locations. Changed to INNER JOIN with WHERE clause.
  -- ================================================================
  ...
  -- ================================================================
  -- FIFO deduction per item across pallets
  -- ================================================================
  FOR v_item IN
    SELECT *
  ...
  LOOP
    ...
    -- ================================================================
    -- FIX: directly decrement item_locations for the warehouse.
    -- Previously this relied on a deferred trigger that did not exist.
    -- ================================================================
    ...
    SELECT
      ...
      v_warehouse_loc_id,
      p_admin_user_id,
      il.current_quantity + v_pieces_deducted,  -- value before deduction
      il.current_quantity,                       -- value after deduction
      -v_pieces_deducted,
      'used',
  ...
  -- ================================================================
  -- Partial fulfillment: create remainder ticket
  -- ================================================================
