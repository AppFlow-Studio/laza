...
const getSteps = (role?: string): WizardStep[] => [
	...
	{
		number: 2,
		title: role === 'admin' ? 'Locations' : role === 'super_admin' ? 'Access' : 'Location',
					...
					<div key={step.number} className="relative">
						...
						>
							<div className={cn(
								...
								!isCurrent && !isCompleted && "border-zinc-200 text-zinc-400"
							)}>
							...
							<div className="min-w-0 pt-0.5">
								<p className={cn(
									...
									isCurrent ? "text-indigo-600" : isCompleted ? "text-zinc-900" : "text-zinc-400"
								)}>
									{step.title}
								</p>
								<p className="text-xs text-zinc-400 mt-0.5 leading-tight">{step.description}</p>
							</div>
						</button>

						{!isLast && (
							<div className="absolute left-[1.2rem] top-[2.5rem] w-0.5 h-2.5">
								<div className={cn(
									"w-full h-full rounded-full",
									isCompleted ? "bg-indigo-400" : "bg-zinc-200"
								)} />
							</div>
						)}
					</div>
