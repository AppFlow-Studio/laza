...
export default function AddItemsToStorageSpace({
    ...
    // Super admin sees all items, admin sees only location catalog
    const { data: allItems, isLoading: allItemsLoading } = useItems();
    const { data: catalogItems, isLoading: catalogLoading } =
        useLocationCatalog(locationId);

    const items = isSuperAdmin
        ? allItems
        : catalogItems?.map((c) => c.items).filter(Boolean);
                                        ...
                                        {isSelected && (
                                            <div className="mt-3 pt-3 border-t border-zinc-200 space-y-3">
                                                <div>
                                                    <Label
                                                        htmlFor={`qty-${item.id}`}
                                                        className="text-xs text-zinc-600 mb-1 block"
                                                    >
                                                        Initial Quantity
                                                    </Label>
                                                    <Input
