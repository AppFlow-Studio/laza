...
interface ItemAssignmentStepProps {
    ...
    onBulkToggle?: (itemIds: string[], shouldSelect: boolean) => void;
...
export default function ItemAssignmentStep({
    ...
    // Filter items for display (includes search and category)
    ...
    // Items to select when "Select All" is clicked
    // If category filter is active, select only category-filtered items
    // If no category filter, select all items (only filtered by search if search is active)
    const itemsToSelect = categoryFilter
        ? items?.filter((item) => {
            const itemCategoryId = getCategoryId(item);
            return itemCategoryId === categoryFilter;
        }) || []
        : items?.filter((item) => {
            // If no category filter, apply only search filter if search is active
            if (debouncedSearch) {
                const searchLower = debouncedSearch.toLowerCase();
                return (
                    item.name.toLowerCase().includes(searchLower) ||
    ...
    const allItemsToSelectSelected = itemsToSelect.length > 0 && itemsToSelect.every(item => selectedItems.has(item.id));

    const handleSelectAllFiltered = () => {
        if (onBulkToggle) {
            // Use bulk toggle for better performance
            const itemIds = itemsToSelect.map(item => item.id);
            onBulkToggle(itemIds, !allItemsToSelectSelected);
        } else {
            // Fallback to individual toggles if bulk toggle not available
            if (allItemsToSelectSelected) {
                // Deselect all items that should be selected
                ...
                // Select all items that should be selected
                itemsToSelect.forEach(item => {
                    if (!selectedItems.has(item.id)) {
                        onItemToggle(item.id);
                    }
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    >
                        {allItemsToSelectSelected ? 'Deselect All' : 'Select All'}
                    </Button>
