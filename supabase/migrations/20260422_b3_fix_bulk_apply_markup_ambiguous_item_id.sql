-- Fix: qualify item_id in RETURNING clause to resolve ambiguity between
-- the item_warehouse_pricing table column and the PL/pgSQL return variable.
...
BEGIN
  ...
  upserted AS (
    ...
    SELECT item_id, computed_new_price, v_actor
    FROM targets
    ...
    ON CONFLICT (item_id) DO UPDATE
      SET warehouse_transfer_price = EXCLUDED.warehouse_transfer_price,
          ...
          updated_at               = NOW()
    RETURNING item_warehouse_pricing.item_id, warehouse_transfer_price AS new_price
  )
  SELECT t.item_id, t.old_price, u.new_price
  FROM targets t
  JOIN upserted u USING (item_id);
END; $$;
