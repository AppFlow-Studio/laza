...
export default function PurchaseOrderDetailPage({
    ...
    const { id: warehouseId, poId } = use(params);
    const router = useRouter();

    const { data: po, isLoading } = usePurchaseOrder(poId);
    const deletePO = useDeletePurchaseOrder();
    const { data: pallets = [] } = usePallets(warehouseId, { purchaseOrderId: poId });
            ...
            <div className="bg-white rounded-xl border border-zinc-200 shadow-sm p-6">
                <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <StatusActions poId={po.id} currentStatus={status} warehouseId={warehouseId} />
                    </div>
                </div>
