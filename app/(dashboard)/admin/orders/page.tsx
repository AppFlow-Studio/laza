...
import {
  Package, Plus, ChevronRight, Clock, CheckCircle2, XCircle,
  Loader2, Send, Archive, FileEdit, Truck, Search,
  CalendarDays, LayoutGrid, List, MapPin,
} from "lucide-react";
...
function getTotalBoxes(ticket: RawTicket): number {
  return (
    ticket.order_ticket_items?.reduce((sum, line) => sum + (line.quantity_boxes ?? 0), 0) ?? 0
  );
}
...
const STATUS_CONFIG: Record<
  TicketStatus,
  { label: string; icon: React.ReactNode; badge: string; dot: string; accent: string }
> = {
  draft:      { label: "Draft",      icon: <FileEdit size={10} />,                         badge: "bg-gray-100 text-gray-500",     dot: "bg-gray-300",   accent: "bg-gray-300"   },
  submitted:  { label: "Submitted",  icon: <Send size={10} />,                              badge: "bg-blue-50 text-blue-700",      dot: "bg-blue-500",   accent: "bg-blue-500"   },
  processing: { label: "Processing", icon: <Loader2 size={10} className="animate-spin" />, badge: "bg-yellow-50 text-yellow-700",  dot: "bg-yellow-500", accent: "bg-yellow-400" },
  fulfilled:  { label: "Fulfilled",  icon: <Truck size={10} />,                             badge: "bg-violet-50 text-violet-700",  dot: "bg-violet-500", accent: "bg-violet-500" },
  confirmed:  { label: "Confirmed",  icon: <CheckCircle2 size={10} />,                      badge: "bg-green-50 text-green-700",    dot: "bg-green-500",  accent: "bg-green-500"  },
  rejected:   { label: "Rejected",   icon: <XCircle size={10} />,                           badge: "bg-red-50 text-red-700",        dot: "bg-red-500",    accent: "bg-red-500"    },
  cancelled:  { label: "Cancelled",  icon: <Archive size={10} />,                           badge: "bg-gray-100 text-gray-400",     dot: "bg-gray-200",   accent: "bg-gray-200"   },
};

const ALL_STATUSES: TicketStatus[] = [
  "draft", "submitted", "processing", "fulfilled", "confirmed", "rejected", "cancelled",
];
...
function FilterChip({
  ...
  return (
    ...
    >
      {label}
      <span className={`text-[10px] px-1 py-0.5 rounded ${active ? "bg-white/20 text-white/80" : "bg-gray-100 text-gray-400"}`}>
        {count}
      </span>
    </button>
  );
  ...
  return (
    ...
    >
      <div
        ...
        style={{ gridTemplateColumns: "10px 110px 96px 96px 1fr 148px 20px" }}
      >
        {/* Status dot */}
        ...
        <div className="flex items-center gap-1 text-xs text-gray-400 truncate">
          ...
          <span className="truncate">{ticket.requesting_location?.name ?? "—"}</span>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between items-center py-1 border-b border-gray-50">
              ...
              <span className="text-xs font-semibold text-gray-800">{itemCount}</span>
            </div>
            <div className="flex justify-between items-center py-1 border-b border-gray-50 last:border-0">
              <span className="text-[11px] text-gray-400">Boxes</span>
              <span className="text-xs font-semibold text-gray-800">{totalBoxes}</span>
            </div>
            {ticket.notes && (
  ...
  const filtered = useMemo(() => {
    ...
    return (tickets as RawTicket[]).filter((t) => {
      ...
      const matchSearch =
        ...
        (t.requesting_location?.name ?? "").toLowerCase().includes(search.toLowerCase());
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div
              ...
              style={{ gridTemplateColumns: "10px 110px 96px 96px 1fr 148px 20px" }}
            >
              {["", "Ticket ID", "Date", "Location", "Contents", "Status", ""].map((h, i) => (
                <span
                  key={i}
